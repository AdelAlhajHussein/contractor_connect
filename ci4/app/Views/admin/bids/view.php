...
<div class="reports-container">
    ...
    <p>
        <a class="action-link" href="<?= site_url('admin/bids') ?>">← Back to Bids</a>
    </p>
    ...
    <h2 class="report-section-title">Tasks</h2>

    <?php if (empty($tasks)): ?>
        <p class="report-summary">No tasks for this bid.</p>
    <?php else: ?>
    ...
    <?php if (!in_array($bid['status'], ['accepted', 'rejected', 'withdrawn'], true)): ?>
        <a class="action-link"
           href="<?= site_url('admin/bids/withdraw/' . $bid['id']) ?>"
           onclick="return confirm('Withdraw this bid?')">
            Mark as Withdrawn
        </a>
    <?php else: ?>
        <p class="report-summary"><b>Action locked:</b> bid is <?= esc($bid['status']) ?>.</p>
    <?php endif; ?>
